...
use aya_ebpf_bindings::helpers as helpers_ex;
...
fn process<const MODE: u8>(ctx: &XdpContext) -> u32 {
    ...
    let time = unsafe { gen::bpf_ktime_get_ns() };
        ...
        SEND_VIA_RING_WITH_DELAY | SEND_VIA_RING_WITH_EPOLL => {
            if let Some(mut event) = RING.reserve::<RingEvent>(0) {
                ...
                unsafe {
                    ...
                    if aya_ebpf::helpers::gen::bpf_xdp_load_bytes(
                        ctx.ctx,
                        0,
                        buf_ptr as _,
                        len as u32,
